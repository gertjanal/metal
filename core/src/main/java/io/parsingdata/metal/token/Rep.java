...
public class Rep extends Token {
    ...
    protected ParseResult parseImpl(final String scope, final Environment env, final Encoding enc) throws IOException {
        final ParseResult res = iterate(scope, env.newEnv(env.order.addBranch(this, env.sequenceId + 1), env.input, env.offset), enc);
        return new ParseResult(true, env.newEnv(res.getEnvironment().order.closeBranch(), res.getEnvironment().input, res.getEnvironment().offset));
    }
