...
public class SubTest {
    ...
    public void test() throws IOException {
        ...
        final Environment env = new Environment(new InMemoryByteStream(root));
        final Encoding enc = new Encoding(false, StandardCharsets.UTF_8, ByteOrder.LITTLE_ENDIAN);
        final ParseResult result = DIRECTORY.parse(env, enc);
    ...
    public void test2() throws IOException {
        final byte[] data = {3, 42, 42, 43, 43};

        final Token success = seq(
            def("ref", 1),
            def("value1", 1),
            sub(
                seq(
                    def("value1", 1),
                    def("value2", 1)), ref("ref")
            ),
            def("value2", 1));

        final Environment env = new Environment(new InMemoryByteStream(data));
        final ParseResult result = success.parse(env, enc());
        ...
        final Token fail = seq(
            ...
            def("value2", 1, eqNum(ref("value1"))));

        final ParseResult result2 = fail.parse(env, enc());
